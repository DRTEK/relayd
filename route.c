...
int relayd_rtnl_init(void)
{
	...
	/*START: MUSTAFA: CHANGES TO "RESOLVE ADDRESS ALREADY IN USE" ISSUE */
	printf("setting nl_pid = %d\n",getpid());
	snl_local.nl_pid = getpid();

	if(setsockopt(rtnl_sock.fd, SOL_SOCKET, SO_REUSEADDR, &one, sizeof(one)) < 0) {
		perror("setsockopt(AF_NETLINK:rtnl_sock.fd)");
	}
	/*END: MUSTAFA: CHANGES TO "RESOLVE ADDRESS ALREADY IN USE" ISSUE */

	if (bind(rtnl_sock.fd, (struct sockaddr *) &snl_local, sizeof(struct sockaddr_nl)) < 0) {
		perror("PID:::AF_NETLINK:rtnl_sock.fd bind");	
		close(rtnl_sock.fd);
		return -1;
	}
